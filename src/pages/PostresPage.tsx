...
export function PostresPage({ onNavigate }: PostresPageProps) {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/api/products?category=Postres`)
      .then(res => res.json())
      .then(data => {
        setProducts(data);
        if(data.length > 0) {
    ...
    <>
      <section className="py-16 bg-gradient-to-br from-pink-50 via-rose-50 to-fuchsia-50 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-5xl lg:text-6xl font-bold text-primary mb-4">Postres</h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Deléitate con nuestros exquisitos postres artesanales.</p>
        </div>
      </section>
            ...
            <div className="flex-1">
              {loading ? <div className="text-center py-16">Cargando...</div> : (
                filteredAndSortedProducts.length > 0 ? (
                  ...
                  <div className="text-center py-16"><p>No se encontraron postres.</p><Button onClick={handleClearFilters} variant="outline">Limpiar</Button></div>
                )
              )}
            </div>
          </div>
